    ...
    try {
        ...
        if (EggOrEggless === undefined || Flavour === undefined || User_ID === undefined ||
            ...
            const CustomizeCake = new CustomizeCakeModel({
                ...
                Shape: Shape,
                Weight: NewWeight,
                Theme: Theme?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                Tier: Tier,
                Images: imageUrlList,
                MessageOnTheCake: MessageOnTheCake?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                SpecialRequest: SpecialRequest?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
