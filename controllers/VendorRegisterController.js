                    ...
                    vendorModel.findOne({ Email: Email }, async function (err, result) {
                        if (err) {
                            ...
                            const vendorValidate = new vendorModel({
                                ...
                                Email: Email,
                                Password: Password,
                                VendorName: VendorName?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
        ...
        if (FinalEmail === 'already exist') {
            ...
            vendorModel.findOneAndUpdate({ _id: id }, {
                $set: {
                    ...
                    YearsOfExperienceAsBaker: YearsOfExperienceAsBaker,
                    AadhaarNumber: AadhaarNumber,
                    PANNumber: PANNumber?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                    GSTNumber: GSTNumber?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                    FSSAINumber: FSSAINumber?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                    FSSAIExpiryDate: FSSAIExpiryDate,
                    JobType: JobType,
                    BankName: BankName?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                    Branch: Branch?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                    AccountNumber: AccountNumber,
                    IFSCCode: IFSCCode?.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()),
                    UPIId: UPIId,
                    Ratings: Ratings,
                    AreYouFamiliarOnWorkingWithApps: AreYouFamiliarOnWorkingWithApps,
                    LearningType: LearningType,
